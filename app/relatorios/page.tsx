...
export default function RelatoriosPage() {
  const { clients, appointments, services, serviceVariants, sales, payments, isLoading, error, refreshData } = useData()
  ...
  const getPeriodData = (days: number): PeriodData => {
    const now = new Date()
  ...
  const currentPeriod = useMemo(() => {
    const days = periodFilter === "all" ? 365 * 10 : parseInt(periodFilter)
  ...
  const metrics = useMemo(() => {
    const revenue = currentPeriod.payments.reduce((acc, p) => acc + p.amount, 0)
    const previousRevenue = previousPeriod?.payments.reduce((acc, p) => acc + p.amount, 0) || 0
    ...
    const previousCompletedAppointments = previousPeriod?.appointments.filter(a => a.status === "completed").length || 0
    const appointmentsChange = previousCompletedAppointments > 0 
      ? ((completedAppointments - previousCompletedAppointments) / previousCompletedAppointments) * 100 
      : 0
    ...
    const previousAvgTicket = previousPeriod && previousPeriod.payments.length > 0 
    ...
    const servicesSoldChange = previousServicesSold > 0 
    ...
    const clientRetention = clients.filter(c => c.isClient).length
    const retentionRate = clients.length > 0 ? (clientRetention / clients.length) * 100 : 0
  ...
  const handleExportComprehensive = () => {
    ...
    const paymentsData = Object.entries(metrics.paymentsByMethod).map(([method, amount]) => ({
      ...
      Percentual: `${((amount / metrics.revenue) * 100).toFixed(1)}%`
    }))
    ...
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6">
      <div className="flex flex-col gap-3 sm:gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <Button onClick={handleExportComprehensive} variant="outline" size="default" className="w-full sm:w-auto">
      ...
      <Tabs defaultValue="overview" className="space-y-4 sm:space-y-6">
            ...
            <Card className="p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs sm:text-sm text-muted-foreground">Taxa de Cancelamento</span>
                  <Badge variant={metrics.cancellationRate < 10 ? "default" : metrics.cancellationRate < 20 ? "secondary" : "destructive"} className="text-xs">
                    {metrics.cancellationRate.toFixed(1)}%
                  </Badge>
                </div>
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs sm:text-sm text-muted-foreground">Taxa de Retenção</span>
                  <Badge variant={metrics.retentionRate > 70 ? "default" : metrics.retentionRate > 50 ? "secondary" : "outline"} className="text-xs">
            ...
            <Card className="p-4 sm:p-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  const percentage = currentPeriod.appointments.length > 0
            ...
            {Object.keys(metrics.paymentsByMethod).length === 0 ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                {Object.entries(metrics.paymentsByMethod)
                  .sort(([, a], [, b]) => b - a)
                  .map(([method, amount]) => {
                    const percentage = (amount / metrics.revenue) * 100
                    ...
                    return (
                      <div key={method} className="space-y-2">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <p className="font-semibold text-sm sm:text-lg whitespace-nowrap">{formatCurrency(amount)}</p>
                        </div>
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <div
        ...
        <TabsContent value="clients" className="space-y-4 sm:space-y-6">
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              title="Conversão"
              value={`${metrics.totalClients > 0 ? ((clients.filter(c => c.isClient).length / metrics.totalClients) * 100).toFixed(1) : 0}%`}
              ...
              <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg border bg-card gap-3">
                ...
                <Badge variant="default" className="text-xs">
                  {metrics.totalClients > 0 
                    ? ((metrics.activeClients / metrics.totalClients) * 100).toFixed(0) 
                    : 0}%
                </Badge>
              </div>
              <div className="flex items-center justify-between p-3 sm:p-4 rounded-lg border bg-card gap-3">
        ...
        <TabsContent value="services" className="space-y-4 sm:space-y-6">
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <StatCard
              ...
              value={formatCurrency(metrics.revenue)}
              subtitle={`${currentPeriod.sales.length} vendas`}
              icon={<DollarSign className="h-full w-full text-primary" />}
            />
            <StatCard
              title="Ticket Médio"
              value={formatCurrency(metrics.servicesSold > 0 ? metrics.revenue / metrics.servicesSold : 0)}
              subtitle="Valor médio unitário"
              icon={<Target className="h-full w-full text-primary" />}
            />
          </div>

          <Card className="p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold mb-4">Top 5 Serviços Mais Vendidos</h3>
            {metrics.topServices.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <Package className="h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground mb-3" />
                <p className="text-sm text-muted-foreground">Nenhum serviço vendido no período</p>
              </div>
            ) : (
              <div className="space-y-3 sm:space-y-4">
                {metrics.topServices.map((service, index) => {
                  const percentage = (service.revenue / metrics.revenue) * 100
                  return (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs sm:text-sm text-muted-foreground">
                              {service.quantity} {service.quantity === 1 ? 'venda' : 'vendas'}
                            </p>
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className="font-semibold text-sm sm:text-base">{formatCurrency(service.revenue)}</p>
          ...
          <Card className="p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold mb-4">Variantes de Serviços Cadastradas</h3>
            <div className="space-y-2">
              {serviceVariants.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  <Package className="h-10 w-10 sm:h-12 sm:w-12 text-muted-foreground mb-3" />
                  <p className="text-sm text-muted-foreground">Nenhuma variante de serviço cadastrada</p>
                </div>
              ) : (
                serviceVariants.map(variant => {
                  const service = services.find(s => s.id === variant.serviceId)
                  return (
                    <div key={variant.id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-sm sm:text-base truncate">{service?.name} - {variant.variantName}</p>
                        <p className="text-xs sm:text-sm text-muted-foreground line-clamp-1">{service?.description}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-semibold text-sm sm:text-base">{formatCurrency(variant.price)}</p>
